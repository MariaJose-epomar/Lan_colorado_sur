...
DATA_SECTION
  ...
  !! ad_comm::change_datafile_name("lacsur_op.ctl");
   
  init_matrix Wmed(1,nanos,1,ntallas)
  init_vector Wm_fut(1,ntallas)
...
// los arreglos usados en la proyeccion
 ...
 //MODELO OPERATIVO:DIMENSIONAMIENTO DE NUEMEROS, VECTORES Y MATRICES
 number SO;
 number RO;
 number alfa;
 number beta;	
 ...
 vector sim_bcru(nanos+1,nanos+nanos_proy);
 vector cpue_epsilon_fut(nanos+1,nanos+nanos_proy);
 vector rec_epsilon_fut(nanos+1,nanos+nanos_proy);
 vector bcru_epsilon_fut(nanos+1,nanos+nanos_proy);
 ...
 matrix fut_Ctot_a(nanos+1,nanos+nanos_proy,1,nedades);
 matrix fut_Ctot(nanos+1,nanos+nanos_proy,1,ntallas);
 matrix fut_pflo(nanos+1,nanos+nanos_proy,1,ntallas);
 matrix fut_pcru(nanos+1,nanos+nanos_proy,1,ntallas);
 ...
 matrix keep_F(1,npbr,nanos+1,nanos+nanos_proy);
...
FUNCTION OP_Model
 ...
 dvector epsilon_cpue_fut(nanos+1,nanos+nanos_proy);
 dvector epsilon_bcru_fut(nanos+1,nanos+nanos_proy);
 int upk;
 int k;
 dvariable numyear=1968;
 dvector yrs(nanos+1,nanos+nanos_proy);
 simname="lacsur2019.dat"; //nombre de los datos del estimador
 ...
 for(int l=1;l<=npbr;l++){
	 rv=system("cp lacsur2019_bkp.dat lacsur2019.dat");
	 rv=system("./lacsur2019 -nox -nohess");
 ...
 for(int i=nanos+1;i<=nanos + nanos_proy;i++){
	
 		rec_epsilon_fut(i)=exp(epsilon_rec_fut(i)* cvar(2));
 		cpue_epsilon_fut(i)=exp(epsilon_cpue_fut(i)* 0.2);
 		bcru_epsilon_fut(i)=exp(epsilon_bcru_fut(i)* 0.2);
       
        //cout << "HASTA AQUI" <<  bcru_epsilon_fut<< endl; exit(1); 		
		...
		if(ErrorProc==1){
		Np(1)=alfa*BDp/(beta+BDp);	
		}
		if(ErrorProc==2){
		Np(1)=alfa*BDp/(beta+BDp)*exp(rec_epsilon_fut(i));  					
		}
		
		//Seccion para estimar la F que genera la CBA leida desde el estimador
	    NVflo_fut = (elem_prod(Np,Sflo(nanos)))*Prob_talla; //NVflo_m_fut                   		es un vector  dim(1:ntallas) inicial sin efectos de exp(-dt(2)*Zph) ya                      que no conocemos F (el cual se estimará con Newton-Raphson a          		continuación)	    
		//BMflo_fut = NVflo_fut * Wm_fut; //Biomasa vulnerable 
		expl_biom= NVflo_fut*Wm_fut;
		...
		//Biomasa vulnerable
		NVflo_fut = elem_prod(elem_prod(Np,mfexp(-0.67*Zpbr)),Sflo(nanos))*Prob_talla;
		BMflo_fut(i) = NVflo_fut * Wm_fut; //Biomasa vulnerable 
		
		//Crucero
		NVcru_fut = elem_prod(elem_prod(Np,mfexp(-0.67*(Zpbr))),Scru(nanos))*Prob_talla;
		BMcru_fut(i) = NVcru_fut*Wm_fut;
		//Biomasa total
	    Bp=sum(elem_prod(Np*Prob_talla,Wm_fut));
	    NMDp=elem_prod(Np,mfexp(-0.67*Zpbr))*Prob_talla; //Numero desovante
	    BDp=sum(elem_prod(elem_prod(NMDp,msex),Wm_fut)) ; //biomasa desovante
		
		//======== SIMULA LOS DATOS PARA ACTUALIZAR LA EVALUACION CON EL ESTIMADOR =====
		//CPUE
		if(ErrorObs==1){
		sim_cpue(i) = exp(log_qCPUE)*BMflo_fut(i);
		//Biomasa crucero
		sim_bcru(i) = exp(log_qCru)*BMcru_fut(i);
		}
		if(ErrorObs==2){
		sim_cpue(i) = exp(log_qCPUE)*BMflo_fut(i)*exp(cpue_epsilon_fut(i));
		//Biomasa crucero
		sim_bcru(i) = exp(log_qCru)*BMcru_fut(i)*exp(bcru_epsilon_fut(i));
		}
		...
		for(int j=1;j<=nedades;j++){//captura por edad
			fut_Ctot_a(i,j) = (F_fut(i,j)/Zpbr(j))*Np(j)*(1.-Sp(j));
		}
		fut_Ctot(i) = fut_Ctot_a(i)*Prob_talla;
		for(int j =1; j<=ntallas;j++){
			fut_pflo(i,j)=fut_Ctot(i,j)/sum(fut_Ctot(i)+1e-10);
			fut_pcru(i,j)= NVcru_fut(j)/sum(NVcru_fut+1e-10);		
		}
		...
		simdata << edad_ini << endl;
		simdata <<"#Delta edad"<<endl;
		simdata <<delta_edad <<endl;		
		...
		simdata << " "<< yrs_fut(k) << "  " << sim_bcru(k) << "  " << 0.2 << "  " << sim_cpue(k) << "  " << 0.2 << "  " << C_fut(l,k) << "  " << 0.1 << "  " << endl;
		}
		simdata << "#Estructura de tallas historica" << endl;
		simdata << "# flota " << endl;
		simdata << Cl << endl; //TODO: revisar
		simdata << "# flota simulados " << endl;
		for(k=nanos+1;k<=upk;k++){
			simdata << fut_pflo(k) << endl;
		}
	    ...
	    simdata << "#Peso medio a la talla" << endl;
	    simdata << Wmed << endl;
		...
		des_Btot(l,i)=Bp; //Biomasa total
		des_SSBt(l,i)=BDp; //Biomasa desovante
		des_Rt(l,i) = Np(1); //Reclutamiento 
...
FUNCTION void do_Newton_Raphson_for_mortality(int i)
		 ...
		 F_tmp(i)=Fnew*Sflo_fut;
		 
FUNCTION get_stock_recluta
 		//CALCULA S0 at F=0
		RO=mfexp(log_Rmed); //"DUDA"
		SO = SDo2;
		if(SrType==1){
		alfa = (SO/RO)*((1-h)/(4*h));
		beta = (5*h-1)/(4.*h*RO);
...
REPORT_SECTION
  ...
  rep(Tallas*trans(pobs));
  rep(Tallas*trans(ppred));
  //report << "Lc_obs_pred" << endl;
  rep(Tallas*trans(pobs_cru));
  rep(Tallas*trans(ppred_cru));
